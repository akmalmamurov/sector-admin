...
export const ProductCharacterLink = ({ control, handleNext, handleBack, linkData, setValue }: Props) => {
  const { fields: titleFields, append: appendTitle, remove: removeTitle } = useFieldArray({
    control,
    name: "characteristics",
  });
  useEffect(() => {
    if (linkData?.characteristics) {
      const transformed = linkData.characteristics.map((item) => ({
        title: item.title,
      ...
      setValue("characteristics", transformed);
    }
  }, [linkData, setValue]);
...
export default ProductCharacterLink;
